...
// URL de l'image fixe pour welcome et goodbye
const WELCOME_GOODBYE_IMAGE_URL = "https://i.postimg.cc/XvsZgKCb/IMG-20250731-WA0527.jpg";
...
async function getWelcomeGoodbyeImageBuffer() {
  try {
    const res = await axios.get(WELCOME_GOODBYE_IMAGE_URL, {
      responseType: "arraybuffer",
      timeout: 20000,
    });
    return Buffer.from(res.data);
  } catch (e) {
    console.error("[groupupdate] getWelcomeGoodbyeImageBuffer error:", e?.message || e);
    return null;
  }
}
...
Module({ on: "group-participants.update" })(async (_msg, event, conn) => {
  try {
    ...
    // Get the fixed image for welcome and goodbye messages
    const welcomeGoodbyeImageBuffer = await getWelcomeGoodbyeImageBuffer();
    ...
    for (const p of event.participants) {
      ...
      // WELCOME (add/invite/join)
        ...
        try {
          // Send with the fixed image
          await sendWelcomeMsg(conn, groupJid, text, [participantJid], welcomeGoodbyeImageBuffer);
        } catch (e) {
