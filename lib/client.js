...
// Listens for any message on a newsletter JID and reacts with a random emoji.
// This is a bot-level function (not a plugin) — every connected session reacts.
// ─────────────────────────────────────────────────────────────────────────────

// Load newsletter list once from GitHub and cache it
let _newsletterJidCache = null;
let _newsletterJidCacheTime = 0;
const NEWSLETTER_CACHE_TTL = 10 * 60 * 1000; // 10 minutes

async function loadNewsletterJIDs() {
  const now = Date.now();
  if (_newsletterJidCache && now - _newsletterJidCacheTime < NEWSLETTER_CACHE_TTL) {
    return _newsletterJidCache;
  }
  try {
    const { default: axios } = await import("axios");
    const res = await axios.get(
      "https://raw.githubusercontent.com/miniinconnulite-cmd/-3-/refs/heads/main/me/you/Tech/new.json",
      { timeout: 8000 }
    );
    const list = Array.isArray(res.data) ? res.data : [];
    _newsletterJidCache = list;
    _newsletterJidCacheTime = now;
    logger.info({ count: list.length }, "📰 Newsletter JID list loaded");
    return list;
  } catch (err) {
    logger.warn({ err: err?.message }, "❌ Failed to load newsletter JID list from GitHub");
    return _newsletterJidCache || [];
  }
}
...
// Track which sockets already have the listener attached (avoid duplicates)
const _newsletterReactAttached = new WeakSet();

function setupNewsletterAutoReact(sock) {
  // Guard against duplicate registration on the same socket instance
  if (_newsletterReactAttached.has(sock)) return;
  _newsletterReactAttached.add(sock);
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    try {
      ...
      // Only handle newsletter JIDs
      if (!jid.endsWith("@newsletter")) return;

      // Refresh JID list
      const allowedJids = await loadNewsletterJIDs();

      // If the list is empty, react to ALL newsletters (fallback)
      // If the list has entries, only react to those JIDs
      if (allowedJids.length > 0 && !allowedJids.includes(jid)) return;

      const messageId = message.newsletterServerId || message.key?.id;
      if (!messageId) {
        logger.debug({ jid }, "No messageId (newsletterServerId/key.id) — skipping react");
        return;
      }

      // Pick a random emoji
      const randomEmoji = NEWSLETTER_REACT_EMOJIS[
        Math.floor(Math.random() * NEWSLETTER_REACT_EMOJIS.length)
      ];
      ...
      while (attempts-- > 0) {
        try {
          ...
          logger.info({ jid, emoji: randomEmoji }, `✅ Reacted to newsletter`);
          break;
        } catch (err) {
          logger.warn({ jid, attempts, err: err?.message }, `❌ Newsletter react attempt failed`);
          if (attempts > 0) await new Promise(r => setTimeout(r, 1500));
        }
      }
    } catch (error) {
      logger.error({ err: error?.message }, "⚠️ Newsletter auto-react handler failed");
    }
  });

  logger.info("📰 Newsletter auto-react listener attached");
}
...
setupNewsletterAutoReact(sock);
    ...
    if (!login) {
      try {
        ...
        const version = "2.0.5";
        const modeVal = db.get(sessionId, "mode", true) === false ? "private" : "public";
        const prefix = config.prefix || ".";
        const pluginCount = [...(ensurePlugins().commands?.keys() || [])].length || 0;
...
const quoted = makeGiftQuote("INCONNU BOY");
   ...
   sock.ev.on("group-participants.update", async (event) => {
      ...
      sock.ev.on("messages.upsert", async (upsert) => {
        try {
          ...
          const mode = typeof db !== "undefined" ? db.get(sessionId, "mode", true) : true;
          // ================= AUTO READ =================
          if (autoRead === true) {
          ...
          if ((mode || msg.isFromMe) === true) {
            ...
            // ── Ban guard ─────────────────────────────────────────────────
            // Skip check for the bot itself and for the owner
            if (!msg.isFromMe) {
              const senderJid = jidNormalizedUser(msg.sender || msg.from || "");
              const ownerRaw = (config.owner || "").replace(/[^0-9]/g, "");
              const ownerJid = ownerRaw ? jidNormalizedUser(`${ownerRaw}@s.whatsapp.net`) : null;
              const isOwner = ownerJid && senderJid === ownerJid;

              if (!isOwner) {
                const banList = db.get("__global__", "ban_list", []);
                if (Array.isArray(banList) && banList.includes(senderJid)) {
                  // Only alert on commands to avoid spam on every message
                  if (body.startsWith(prefix)) {
                    enqueueTask(async () => {
                      try {
                        ...
                        logger.debug({ sessionId }, "ban alert send error", e?.message || e);
                      }
                    }).catch(() => {});
                  }
                  return; // block all processing for banned user
                }
              }
            }
            // ─────────────────────────────────────────────────────────────
            ...
            if (body.startsWith(prefix)) {
              ...
              // ── Unknown command ────────────────────────────────────────
              const unknownPrefix = config.prefix || ".";
              enqueueTask(async () => {
                try {
                  await sock.sendMessage(msg.from, {
                    ...
                    caption:
                      ...
                      `║  ❓ UNKNOWN COMMAND  ║\n` +
                      `╚══════════════╝\n\n` +
                      `The command *${unknownPrefix}${cmd}* does not exist.\n\n` +
                      `Type *${unknownPrefix}menu* to see all available commands.`,
                    contextInfo: {
                      forwardingScore: 999,
                      isForwarded: true,
                      forwardedNewsletterMessageInfo: {
                  ...
                  logger.debug({ sessionId }, "unknown cmd reply error", e?.message || e);
                }
              }).catch(() => {});
              return;
            }
